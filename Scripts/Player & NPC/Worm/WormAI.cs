        ...
        public class WormAI : MonoBehaviour {
        	...
        	void AttackPlayer() {
        		...
        		if (GameObject.FindGameObjectWithTag ("Player") && Vector3.Distance (transform.position, GameObject.FindGameObjectWithTag ("Player").transform.position) < meleeAttackDistance) {
        			if (alwaysMaxDamage) {
                        GameObject.FindGameObjectWithTag ("Player").transform.root.GetComponent<Health> ().ApplyDamage (maxDamage);
        			} else {
                        GameObject.FindGameObjectWithTag ("Player").transform.root.GetComponent<Health> ().ApplyDamage (Random.Range(minDamage, maxDamage));
        			}
        			if (groundHitVFX) {
