...
//				player.GetComponent<Animator> ().Play ("", -1, 0.0f);
	...
	public void StartCutScene() {
		...
		if (playerCanMove == false) {
			if (player.GetComponent<MovementController> ()) {
				player.GetComponent<MovementController> ().moveLocked = true;
			}
			if (player.GetComponent<MovementController> ()) {
				player.GetComponent<MovementController> ().groundLocked = true;
			}
